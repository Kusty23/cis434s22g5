...
class Timer {
  ...
  getMinutes(timeId) {
    return document.getElementById(timeId).childNodes[1].nodeValue;
  }
}



console.log(document.getElementById("min1").childNodes[1].nodeValue);


// Create an instance of the timer for each player
var p1Time = new Timer("min1", document.getElementById("min1").childNodes[1].nodeValue);
var p2Time = new Timer("min2", document.getElementById("min2").childNodes[1].nodeValue);
    ...
    var timerId = setInterval(function() {
        ...
        if (currentPlayer === 1) {
            if (playing) {
                ...
                document.getElementById("sec1").childNodes[1].nodeValue = padZero(p1Sec);
                document.getElementById("min1").childNodes[1].nodeValue = padZero(p1Time.minutes);
                if (p1Sec === 0) {
        ...
        //Player 2
            if (playing) {
                ...
                document.getElementById("sec2").childNodes[1].nodeValue = padZero(p2Sec);
                document.getElementById("min2").childNodes[1].nodeValue = padZero(p2Time.minutes);
                if (p2Sec === 0) {
                    //If minutes and seconds are zero stop timer with the clearInterval method
